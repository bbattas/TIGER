...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            ...
            "Requires a CSV or Parquet file of grain-pair quaternion misorientations with columns: "
    ...
    gen.add_argument("--misorientation", "-m", type=str, required=True,
                     help="Partial path / glob pattern to locate the misorientation CSV or Parquet file.")
    gen.add_argument("--miso-format",
                     choices=("csv", "parquet", "auto"),
                     default="auto",
                     dest="miso_format",
                     help=(
                         "'auto' detects format by file extension. "
                         "'csv' or 'parquet' force a specific format."
                     ))
...
# ─────────────────────────────────────────────
#  Misorientation file finder
#  Returns (resolved_path, detected_format)
# ─────────────────────────────────────────────

def find_misorientation_file(partial: str, fmt: str = "auto") -> tuple[Path, str]:
    """
    Locate the misorientation file (CSV or Parquet).

    Parameters
    ----------
    partial : str
        Exact path or glob fragment supplied by the user.
    fmt : str
        'auto'    - detect by extension (.parquet → parquet, .csv → csv).
        'csv'     - only search for CSV.
        'parquet' - only search for Parquet.

    Returns
    -------
    (path, format_string)   where format_string is 'csv' or 'parquet'.
    """
    exact = Path(partial)
    if exact.is_file():
        detected = _detect_format(exact, fmt)
    ...
    raise FileNotFoundError(
        f"No misorientation file found matching '{partial}' "
        f"(format='{fmt}'). Searched cwd and one level down."
    )


def _detect_format(path: Path, hint: str) -> str:
    """Resolve format from hint or file extension."""
    ...
    if suffix == ".csv":
        return "csv"
    raise ValueError(
        f"Cannot detect misorientation file format from extension '{suffix}'. "
        f"Use --miso-format csv or --miso-format parquet explicitly."
    )

# ─────────────────────────────────────────────
#  Misorientation loaders
# ─────────────────────────────────────────────

def load_misorientation(csv_path: Path, log) -> dict[tuple[int, int], dict]:
    """
    Parse CSV with columns: i, j, angle_deg, ax_x, ax_y, ax_z.
    Loads the entire file into a dict — suitable for small/medium CSV files.
...
def load_misorientation_parquet(
    ...
    requested neighbor_pairs.  Memory cost is proportional to the number
    of active pairs at this timestep, not to the total file size.

    Strategy
    --------
    1. Collect the union of all grain IDs present in neighbor_pairs.
    2. Use a Parquet row-group filter on column 'i' to pull a candidate
       subset (fast columnar read — avoids loading the whole file).
    3. Exact-match against the full pair set in memory.

    Returns the same dict format as load_misorientation().
    """
    ...
    # All grain IDs that appear in at least one neighbor pair
    grain_ids = set()
    for (a, b) in neighbor_pairs:
        grain_ids.add(a)
        grain_ids.add(b)

    # Parquet predicate push-down: only deserialise row groups where 'i' is
    # in our grain-id set.  This alone can cut I/O dramatically for large files.
    ...
    # Exact match: keep only rows whose (i,j) canonical pair is needed
...
def compute_fmiso_for_pairs(
    ...
    Returns
    -------
    pairs_found  : list of (i, j) tuples that had data in the source file
    fmiso_values : np.ndarray of f_miso values aligned with pairs_found
    missing      : list of (i, j) tuples absent from the source file
    """
    pairs_found = []
    fmiso_list  = []
    missing     = []
    ...
    log.info(
        (
            ...
            f"missing_from_source={len(missing)}, "
...
def write_hdf5_frame(
    ...
    if len(pairs_found) > 0:
        pairs_arr = np.array(pairs_found, dtype=np.int64)

        angle_arr = np.array(
            [misorientation[p]["angle_deg"] for p in pairs_found],
            dtype=np.float64,
        )
        polar_arr = np.array(
            [
                np.arccos(np.clip(
                    misorientation[p]["ax_z"] /
                    np.maximum(np.sqrt(
                        misorientation[p]["ax_x"]**2 +
                        misorientation[p]["ax_y"]**2 +
                        misorientation[p]["ax_z"]**2
                    ), 1e-12),
                    -1.0, 1.0,
                ))
...
def main():
    ...
    # ── Locate misorientation file and detect format ─────────────────────────
    miso_path, miso_fmt = find_misorientation_file(args.misorientation, args.miso_format)
    log.warning(f"Misorientation file: {miso_path}  (format={miso_fmt})")

    # ── For CSV: load once upfront (small enough to hold in memory) ──────────
    # ── For Parquet: defer loading to inside the per-step loop ───────────────
    misorientation_csv: dict[tuple[int, int], dict] | None = None
    if miso_fmt == "csv":
        misorientation_csv = load_misorientation(miso_path, log)
        log.warning(f"  Loaded {len(misorientation_csv)} pairs from CSV.")
        ...
        try:
            with ExodusBasics(exofile) as exo:
                ...
                if args.hdf5:
                    ...
                    h5_handle.attrs["misorientation_file"] = str(miso_path)
                    h5_handle.attrs["misorientation_fmt"]  = miso_fmt
                ...
                try:
                    for frame_idx, step in enumerate(frame_iter):
                        ...
                        # ── 2. Load misorientation data ──────────────────────
                        #   CSV  → already loaded; use the cached dict directly.
                        #   Parquet → load only the pairs we need for this step.
                        if miso_fmt == "csv":
                            misorientation = misorientation_csv
                        else:
                            misorientation = load_misorientation_parquet(
                                miso_path, neighbor_pairs, log
                            )

                        # ── 3. Compute f_miso for each GB pair ───────────────
