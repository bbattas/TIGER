...
def plot_gb_curvature_debug(C: np.ndarray, P: np.ndarray, gb_dict: dict,
    ...
    ax = axes[0]
    im0 = ax.imshow(C0, origin='lower', cmap='viridis', interpolation='nearest')
    ...
    # --- Panel 2: Boundary and junction pixel overlay ---
    ax = axes[1]
    ax.imshow(C0, origin='lower', cmap='gray', alpha=0.3, interpolation='nearest')
    ...
    if len(junction_pixels) > 0:
        ...
        for (ji, jj) in junction_pixels[::max(1, len(junction_pixels) // 20)]:
            circle = plt.Circle((jj, ji), TJ_distance_max,
    ...
    # --- Panel 6: Inclination quiver at boundary pixels ---
    ax = axes[5]
    ax.imshow(C0, origin='lower', cmap='gray', alpha=0.3, interpolation='nearest')
    ...
    if len(boundary_pixels) > 0:
        ...
        for (i, j) in boundary_pixels[::stride]:
            ...
            if mag > 0:
                xs.append(j)   # j is the column = horizontal = x axis for imshow
                ys.append(i)   # i is the row    = vertical   = y axis for imshow
                us.append(dx)
                vs.append(dy)
        ...
        try:
            with ExodusBasics(exofile) as exo:
                ...
                if args.debug_plot or args.verbose >= 2:
                    ...
                    # Find the frame tuple whose step matches the selected target step
                    target_tuple = next(
                        (f for f in frames_data if f[0] == step), frames_data[-1]
                    )
                    _, _, P0_dbg, C_dbg, P_dbg, gb_dict_dbg, bp_dbg, jp_dbg = target_tuple
                    # Shape diagnostics
                    log.info("--- Debug Frame Shape Info ---")
                    log.info(f"  P0 shape:              {P0_dbg.shape}  (ny, nx)")
                    log.info(f"  C  shape:              {C_dbg.shape}  (2, ny, nx) — C[0]=grain ID, C[1]=curvature")
                    log.info(f"  P  shape:              {P_dbg.shape}  (3, ny, nx) — P[0]=grain ID, P[1]=dx, P[2]=dy")
                    log.info(f"  boundary_pixels shape: {bp_dbg.shape}  (N, 2)")
                    log.info(f"  junction_pixels shape: {jp_dbg.shape}  (M, 2)")
                    log.info(f"  gb_dict entries:       {len(gb_dict_dbg)}")
                    log.info(f"  unique grains in P0:   {len(np.unique(np.rint(P0_dbg).astype(int)))}")
                    log.info(' ')
                    log.info("  * nx and ny are mesh coordinate specific here, not the VECTOR internal (nx,ny) = P0.shape")
                    log.info("------------------------------")
