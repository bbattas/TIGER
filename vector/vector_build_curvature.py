    ...
    for cnt,exofile in enumerate(exo_files):
        ...
        try:
            with ExodusBasics(exofile) as exo:
                ...
                # MANUAL TESTING for new content
                nx = P0.shape[0]
                ny = P0.shape[1]
                # ng = np.max(P0)
                ng = int(np.nanmax(P0)) - int(np.nanmin(P0)) + 1
                ...
                C1 = smooth_class.get_C()
                P1 = smooth_class.get_P()
                sites = smooth_class.get_all_gb_list()
                sites_together = []
                for id in range(len(sites)): sites_together += sites[id]

                debug_plot(P0,sites_together,stem)
                debug_quiver(P1,sites_together,xcen,ycen,stem,normalize=True,scale=0.3)
                plot_gb_curvature_debug(C1, gb_dict, boundary_pixels, junction_pixels,
