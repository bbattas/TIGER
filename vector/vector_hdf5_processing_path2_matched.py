...
Post-processing script for multi-frame HDF5 files produced by the VECTOR
HDF5 generation scripts.  Implements GBE, inclination, curvature, and GB
velocity analyses.

The velocity path in this revision intentionally matches the filtering and
classification conventions used by GB_velocity_anti_curvature-iso.py:
  - current-frame-only GB filtering
  - raw next-frame pair persistence check
  - raw pre-TJ GB area for MIN_AREA and velocity normalization
  - zero-product observations omitted from normal/anti-curvature subsets
  - Path-2-style 00100 anti-curvature filter enabled by default
"""
...
# HDF5 gb_dict value layout:
# [avg_curvature, tj_filtered_gb_area, grain_id1, grain_id2, raw_gb_area]
GB_AVG_CURVATURE = 0
GB_TJ_FILTERED_AREA = 1
GB_GRAIN_ID1 = 2
GB_GRAIN_ID2 = 3
GB_RAW_AREA = 4
...
def parse_args() -> argparse.Namespace:
    ...
    gbe.add_argument(
        "--min-grain-area", "--min-area", dest="min_grain_area",
        type=int, default=0, metavar="N",
        help=(
            "Optional minimum total pixel area of each participating grain. "
            "Default 0 disables this filter. --min-area is retained as a "
            "backward-compatible alias. This is not Path 2 MIN_AREA."
        ),
    )
    gbe.add_argument(
        "--min-curvature", type=float, default=0.0, metavar="F",
        help=(
            "Minimum absolute TJ-filtered average GB curvature for keeping an "
            "entire ij GB pair. Path 2 commonly used 0 or 0.0182."
        ),
    )
    gbe.add_argument(
        "--min-gb-area", type=int, default=50, metavar="N",
        help=(
            "Minimum raw GB pixel count before TJ-distance exclusion. This "
            "matches MIN_AREA in GB_velocity_anti_curvature-iso.py."
        ),
    )
    gbe.add_argument(
        "--min-tj-filtered-gb-area", type=int, default=0, metavar="N",
        help=(
            "Optional minimum GB pixel count after TJ-distance exclusion. "
            "Default 0 disables this extra filter; Path 2 does not use it."
        ),
    )
    gbe.add_argument(
    ...
    tog.add_argument(
        "--isolated-antic-filter",
        action=argparse.BooleanOptionalAction,
        default=True,
        help=(
            "Apply the Path 2 five-interval 00100 filter to isolated "
            "anti-curvature events. Disable with --no-isolated-antic-filter."
        ),
    )
    # Backward-compatible opt-in spelling from older versions. The new option
    # is already enabled by default, so this alias is intentionally hidden.
    tog.add_argument(
        "--drop-isolated-antic",
        dest="isolated_antic_filter",
        action="store_true",
    ...
    for option_name, option_value in (
        ("--min-grain-area", args.min_grain_area),
        ("--min-gb-area", args.min_gb_area),
        ("--min-tj-filtered-gb-area", args.min_tj_filtered_gb_area),
        ("--min-curvature", args.min_curvature),
    ):
        if option_value < 0:
            p.error(f"{option_name} must be non-negative, got {option_value}.")
...
class VelocityObservation:
    """One Path-2-compatible GB observation for one frame interval."""
    ...
    dV_backward: int
    raw_gb_area: float
    tj_filtered_gb_area: float

    @property
    def velocity_normalization_length(self) -> float:
        return self.raw_gb_area / 2.0

    @property
    def reported_area(self) -> float:
        # Path 2 stores half of the TJ-filtered valid boundary-pixel count
        # for its area-based outputs, separately from velocity normalization.
        return self.tj_filtered_gb_area / 2.0

    @property
    def behavior_product(self) -> float:
        return self.avg_curvature * self.velocity

    @property
    def is_anti_curvature(self) -> bool:
        return self.behavior_product < 0.0
    ...
    with h5py.File(filepath, "r") as hf:
        ...
        if "provenance" in hf:
            ...
            provenance = {
                ...
                "spatial_transpose": (bool(prov["spatial_transpose"][()])
                                if "spatial_transpose" in prov else False),
            }
...
def filter_gb_dict(
    gb_dict: dict[tuple[int, int], np.ndarray],
    *,
    min_grain_area: float = 0.0,
    min_curvature: float = 0.0,
    min_gb_area: int = 0,
    min_tj_filtered_gb_area: int = 0,
    small_grain_set: set[int] | None = None,
    log: logging.Logger | None = None,
    label: str = "",
) -> dict[tuple[int, int], np.ndarray]:
    """Filter entire GB pairs using explicit, non-overloaded area meanings.

    Expected ``gb_dict[pair_id]`` schema::

        [avg_curvature, tj_filtered_gb_area,
         grain_id1, grain_id2, raw_gb_area]

    Filters
    -------
    min_grain_area
        Optional total grain-pixel-area filter. A pair is removed when either
        participating grain is in ``small_grain_set``. This is not used by the
        Path 2 reference when set to zero.
    min_curvature
        Minimum absolute TJ-filtered average GB curvature.
    min_gb_area
        Minimum raw GB pixel count before TJ-distance exclusion. This matches
        Path 2 ``MIN_AREA``.
    min_tj_filtered_gb_area
        Optional minimum valid GB pixel count after TJ-distance exclusion.
    """
    ...
    for pair_id, data in gb_dict.items():
        avg_curv = float(data[GB_AVG_CURVATURE])
        tj_filtered_gb_area = float(data[GB_TJ_FILTERED_AREA])
        grain_id1 = int(data[GB_GRAIN_ID1])
        grain_id2 = int(data[GB_GRAIN_ID2])
        raw_gb_area = float(data[GB_RAW_AREA])
        ...
        if abs(avg_curv) < min_curvature:
            n_curv += 1
            continue
        if raw_gb_area < min_gb_area:
            n_raw_gb_area += 1
            continue
        if tj_filtered_gb_area < min_tj_filtered_gb_area:
            n_tj_gb_area += 1
            continue
        out[pair_id] = data
    ...
    if log is not None:
        ...
        log.warning(
            ...
            f"{n_grain_area} removed by grain_area<{min_grain_area}, "
            f"{n_curv} removed by |avg_curvature|<{min_curvature}, "
            f"{n_raw_gb_area} removed by raw_gb_area<{min_gb_area}, "
            f"{n_tj_gb_area} removed by "
            f"tj_filtered_gb_area<{min_tj_filtered_gb_area}."
        )
    return out
...
def filter_frames_gb_dicts(
    frames: list[FrameData],
    *,
    min_grain_area: float,
    min_curvature: float,
    min_gb_area: int = 0,
    min_tj_filtered_gb_area: int = 0,
    log: logging.Logger,
) -> list[FrameData]:
    """Return frame copies filtered for GBE and other same-frame analyses.

    Velocity deliberately does not use these filtered frame copies. Its
    current-frame filters and raw next-frame persistence check are applied in
    :func:`compute_gb_velocity_one_interval`.
    """
    filtered: list[FrameData] = []
    for idx, frame in enumerate(frames):
        grain_areas = compute_grain_areas(frame.P0)
        small_grain_set = build_small_grain_set(
            grain_areas, min_area=int(min_grain_area)
        )
        log.info(
            f"  Frame {idx}: {len(small_grain_set)} grains below "
            f"min_grain_area={min_grain_area} px out of "
            f"{len(grain_areas)} total grains."
        )
        filtered.append(replace(
            ...
            gb_dict=filter_gb_dict(
                ...
                min_tj_filtered_gb_area=min_tj_filtered_gb_area,
...
def compute_gb_velocity_one_interval(
    ...
    *,
    timestep_index: int,
    min_grain_area: int = 0,
    min_curvature: float = 0.0,
    min_gb_area: int = 0,
    min_tj_filtered_gb_area: int = 0,
    log: logging.Logger | None = None,
) -> dict[tuple[int, int], VelocityObservation]:
    """Compute Path-2-compatible GB velocities for one frame interval.

    Only the current frame is filtered. Pair persistence is checked against
    the raw, unfiltered ``frame_next.gb_dict``. Velocity is normalized using
    the current frame's raw pre-TJ GB area::

        velocity = dV / dt / (raw_gb_area / 2)
    """
    if log is None:
        log = logging.getLogger("VHP")
    ...
    small_grain_set: set[int] = set()
    if min_grain_area > 0:
        small_grain_set = build_small_grain_set(
            compute_grain_areas(frame_current.P0), min_area=min_grain_area
        )

    results: dict[tuple[int, int], VelocityObservation] = {}
    n_grain_area = 0
    n_curvature = 0
    n_raw_gb_area = 0
    n_tj_gb_area = 0
    n_missing_next = 0
    n_invalid_raw_area = 0

    for pair_id, data_current in frame_current.gb_dict.items():
        avg_curvature = float(data_current[GB_AVG_CURVATURE])
        tj_filtered_gb_area = float(data_current[GB_TJ_FILTERED_AREA])
        grain_id1 = int(data_current[GB_GRAIN_ID1])
        grain_id2 = int(data_current[GB_GRAIN_ID2])
        raw_gb_area = float(data_current[GB_RAW_AREA])

        # Path 2 filters the current-frame observation only.
        if grain_id1 in small_grain_set or grain_id2 in small_grain_set:
            n_grain_area += 1
            continue
        if raw_gb_area < min_gb_area:
        ...
        # Path 2 checks pair existence in the raw next-frame dictionary. The
        # next-frame pair is not required to pass its own area/curvature filter.
        if pair_id not in frame_next.gb_dict:
            n_missing_next += 1
            continue

        if raw_gb_area <= 0.0:
            n_invalid_raw_area += 1
            continue

        dV, dV_forward, dV_backward = compute_dV_split(
            P0_current, P0_next, grain_id1, grain_id2
        )
        velocity = dV / dt / (raw_gb_area / 2.0)

        results[pair_id] = VelocityObservation(
            pair_id=pair_id,
            timestep_index=timestep_index,
            current_step=frame_current.step,
            next_step=frame_next.step,
            grain_id1=grain_id1,
            grain_id2=grain_id2,
            ...
            raw_gb_area=raw_gb_area,
            tj_filtered_gb_area=tj_filtered_gb_area,
        )

    log.info(
        f"  Velocity [{frame_current.step}->{frame_next.step}]: "
        f"{len(results)}/{len(frame_current.gb_dict)} pairs computed; "
        f"removed current-frame grain_area={n_grain_area}, "
        f"raw_gb_area={n_raw_gb_area}, "
        f"tj_filtered_gb_area={n_tj_gb_area}, "
        f"curvature={n_curvature}; "
        f"missing from raw next frame={n_missing_next}; "
        f"invalid raw area={n_invalid_raw_area}."
    )
    return results
...
def compute_gb_velocity_averaged(
    ...
    *,
    ...
    min_gb_area: int = 0,
    min_tj_filtered_gb_area: int = 0,
    all_interval_results: list[dict[tuple[int, int], VelocityObservation]] | None = None,
    log: logging.Logger | None = None,
) -> pd.DataFrame:
    """Compute one averaged velocity row per GB pair.

    ``all_interval_results`` may be supplied from
    :func:`accumulate_velocity_flat_lists` to avoid recomputing dV.
    """
    if log is None:
        log = logging.getLogger("VHP")
    if len(frames) < 2:
    ...
    if all_interval_results is None:
        all_interval_results = []
        for i in range(len(frames) - 1):
            all_interval_results.append(compute_gb_velocity_one_interval(
    ...
    accum: dict[tuple[int, int], list[VelocityObservation]] = {}
    for interval_results in all_interval_results:
        for pair_id, observation in interval_results.items():
            accum.setdefault(pair_id, []).append(observation)

    rows = []
    for pair_id, observations in accum.items():
        velocities = [obs.velocity for obs in observations]
        anti_curv = [obs.is_anti_curvature for obs in observations]
        last = observations[-1]
        rows.append({
            "pair_id": pair_id,
            "grain_id1": last.grain_id1,
            "grain_id2": last.grain_id2,
            ...
            "area": last.reported_area,
            "tj_filtered_gb_area": last.tj_filtered_gb_area,
            "raw_gb_area": last.raw_gb_area,
            "velocity_normalization_length": last.velocity_normalization_length,
            "dV_forward_total": sum(obs.dV_forward for obs in observations),
            "dV_backward_total": sum(obs.dV_backward for obs in observations),
    ...
    if df.empty:
        log.warning(
            "\033[31mVelocity:\033[0m Velocity DataFrame is empty — all GB "
            "pairs were filtered out. Check --min-gb-area, "
            "--min-curvature, and pair persistence."
        )
        return pd.DataFrame(columns=[
            "pair_id", "grain_id1", "grain_id2", "velocity_mean",
            "velocity_std", "n_intervals", "avg_curvature", "area",
            "tj_filtered_gb_area", "raw_gb_area",
            "velocity_normalization_length", "dV_forward_total",
            "dV_backward_total", "anti_curvature_fraction",
        ])
...
def accumulate_velocity_flat_lists(
    ...
    *,
    ...
    """Accumulate current-filtered/raw-next-frame velocity observations."""
    if log is None:
        log = logging.getLogger("VHP")
    ...
    flat = {
        ...
        "all_dV_backward": [],
        "all_areas": [],
        "all_tj_filtered_gb_areas": [],
        "all_raw_gb_areas": [],
        "all_velocity_normalization_lengths": [],
        "all_pair_ids": [],
        "all_grain_id1": [],
        "all_grain_id2": [],
    ...
    for i in range(len(frames) - 1):
        ...
        for pair_id, obs in interval_results.items():
            flat["all_curvatures"].append(obs.avg_curvature)
            flat["all_velocities"].append(obs.velocity)
            flat["all_dV"].append(obs.dV)
            flat["all_dV_forward"].append(obs.dV_forward)
            flat["all_dV_backward"].append(obs.dV_backward)
            flat["all_areas"].append(obs.reported_area)
            flat["all_tj_filtered_gb_areas"].append(obs.tj_filtered_gb_area)
            flat["all_raw_gb_areas"].append(obs.raw_gb_area)
            flat["all_velocity_normalization_lengths"].append(
                obs.velocity_normalization_length
            )
            flat["all_pair_ids"].append(pair_id)
            flat["all_grain_id1"].append(obs.grain_id1)
            flat["all_grain_id2"].append(obs.grain_id2)
            flat["all_timestep_indices"].append(i)
            flat["all_current_steps"].append(obs.current_step)
            flat["all_next_steps"].append(obs.next_step)
            flat["all_behavior_products"].append(obs.behavior_product)
...
def find_isolated_anticurvature_events(
    all_interval_results: list[dict[tuple[int, int], VelocityObservation]],
    log: logging.Logger | None = None,
) -> set[tuple[tuple[int, int], int]]:
    """Return isolated anti-curvature events using Path 2's 00100 kernel.

    Path 2 starts with two empty history sets, so center intervals 0 and 1
    are still testable once two future intervals are available. With ``N``
    velocity intervals, centers 0 through ``N-3`` are tested. The final two
    intervals act as right-hand flanks but are not retained as anti-curvature
    observations.
    """
    if log is None:
        log = logging.getLogger("VHP")
    ...
    for interval in all_interval_results:
        antic_sets.append({
            pair_id
            for pair_id, obs in interval.items()
            if obs.is_anti_curvature
        })

    rejected: set[tuple[tuple[int, int], int]] = set()
    tested_candidates = 0

    # Exact post-hoc equivalent of the streaming five-slot kernel. For n=120,
    # this tests centers 0..117 and uses intervals 118 and 119 only as flanks.
    for center in range(max(0, n - 2)):
        centre_set = antic_sets[center]
        flank_set: set[tuple[int, int]] = set()
        for flank_idx in (center - 2, center - 1, center + 1, center + 2):
            if 0 <= flank_idx < n:
                flank_set |= antic_sets[flank_idx]

        isolated = centre_set - flank_set
        tested_candidates += len(centre_set)
        rejected.update((pair_id, center) for pair_id in isolated)

    log.warning(
        f"  Path 2 00100 filter: {len(rejected)} isolated (pair, interval) "
        f"entries rejected from {tested_candidates} tested anti-curvature "
        f"candidates; final two intervals are anti-curvature flanks only."
    )
    return rejected


def build_path2_velocity_subsets(
    flat: dict,
    *,
    use_isolated_filter: bool = True,
    log: logging.Logger | None = None,
) -> tuple[dict, dict, dict]:
    """Build normal/anti subsets using the reference script's exact tests.

    Classification is based on the raw product ``curvature * velocity``:

    - product > 0: normal-curvature
    - product < 0: anti-curvature
    - product == 0: omitted from both groups

    The lists in ``flat`` are expected to have already passed through
    :func:`apply_curvature_sign_convention`, so selected outputs have positive
    curvature and velocity sign carries normal/anti meaning.
    """
    if log is None:
        log = logging.getLogger("VHP")

    isolated = (
        find_isolated_anticurvature_events(
            flat["all_interval_results"], log=log
        )
        if use_isolated_filter
        else set()
    )

    n_intervals = len(flat["all_interval_results"])
    normc_indices: list[int] = []
    antic_indices: list[int] = []
    omitted_zero_product: list[int] = []
    omitted_isolated: list[int] = []
    omitted_final_two_antic: list[int] = []

    for idx, (product, pair_id, timestep_index) in enumerate(zip(
        flat["all_behavior_products"],
        flat["all_pair_ids"],
        flat["all_timestep_indices"],
    )):
        if product == 0.0:
            omitted_zero_product.append(idx)
            continue

        if product > 0.0:
            normc_indices.append(idx)
            continue

        # Path 2 does not add anti-curvature observations from the final two
        # intervals to GB_id_focus_detail, though they remain 00100 flanks.
        if timestep_index >= max(0, n_intervals - 2):
            omitted_final_two_antic.append(idx)
            continue

        if use_isolated_filter and (pair_id, timestep_index) in isolated:
            omitted_isolated.append(idx)
            continue

        antic_indices.append(idx)

    def extract(indices: list[int]) -> dict:
        return {
            "curvatures": [flat["all_curvatures"][i] for i in indices],
            "velocities": [flat["all_velocities"][i] for i in indices],
            "dV": [flat["all_dV"][i] for i in indices],
            ...
            "tj_filtered_gb_areas": [
                flat["all_tj_filtered_gb_areas"][i] for i in indices
            ],
            "raw_gb_areas": [flat["all_raw_gb_areas"][i] for i in indices],
            "velocity_normalization_lengths": [
                flat["all_velocity_normalization_lengths"][i]
                for i in indices
            ],
            "pair_ids": [flat["all_pair_ids"][i] for i in indices],
            "grain_id1": [flat["all_grain_id1"][i] for i in indices],
            "grain_id2": [flat["all_grain_id2"][i] for i in indices],
            "timestep_indices": [
                flat["all_timestep_indices"][i] for i in indices
            ],
            "current_steps": [flat["all_current_steps"][i] for i in indices],
            "next_steps": [flat["all_next_steps"][i] for i in indices],
        }

    diagnostics = {
        "n_input": len(flat["all_velocities"]),
        "n_normc": len(normc_indices),
        "n_antic": len(antic_indices),
        "n_zero_product_omitted": len(omitted_zero_product),
        "n_isolated_00100_omitted": len(omitted_isolated),
        "n_final_two_antic_omitted": len(omitted_final_two_antic),
        "isolated_filter_enabled": use_isolated_filter,
    }
    log.warning(
        "Path 2 velocity classification: "
        f"{diagnostics['n_normc']} normal, "
        f"{diagnostics['n_antic']} anti, "
        f"{diagnostics['n_zero_product_omitted']} zero-product omitted, "
        f"{diagnostics['n_isolated_00100_omitted']} isolated 00100 omitted, "
        f"{diagnostics['n_final_two_antic_omitted']} final-two anti omitted."
    )

    return extract(normc_indices), extract(antic_indices), diagnostics
...
def save_final_velocity_csv(
    ...
    """Write Path-2-compatible pre-confidence observations for comparison."""
    import csv

    outpath = output_dir / f"{stem}_final_velocity_pre_confidence.csv"
    columns = [
        "timestep_index", "current_step", "next_step",
        "grain_id1", "grain_id2", "pair_id",
        "curvature", "velocity", "dV", "dV_forward", "dV_backward",
        "area", "tj_filtered_gb_area", "raw_gb_area",
        "velocity_normalization_length", "group",
    ]

    def rows(group_data: dict, group_name: str):
        n = len(group_data["curvatures"])
        for i in range(n):
            yield [
                group_data["timestep_indices"][i],
                group_data["current_steps"][i],
                group_data["next_steps"][i],
                group_data["grain_id1"][i],
                group_data["grain_id2"][i],
                group_data["pair_ids"][i],
                group_data["curvatures"][i],
                group_data["velocities"][i],
                group_data["dV"][i],
                group_data["dV_forward"][i],
                group_data["dV_backward"][i],
                group_data["areas"][i],
                group_data["tj_filtered_gb_areas"][i],
                group_data["raw_gb_areas"][i],
                group_data["velocity_normalization_lengths"][i],
                group_name,
            ]
    ...
    with open(outpath, "w", newline="") as f:
        ...
        writer.writerows(rows(normc_flat, "normc"))
        writer.writerows(rows(antic_flat, "antic"))
...
def main() -> None:
    ...
    frames_filtered = filter_frames_gb_dicts(
        ...
        min_tj_filtered_gb_area=args.min_tj_filtered_gb_area,
        log=log,
    )
    ...
    log.warning(
        ...
        f"gb_pairs={len(last_frame.gb_dict)} after current-frame filters."
    )
    ...
    log.warning(
        "Filter convention: --min-gb-area is raw pre-TJ GB pixel count and "
        "matches Path 2 MIN_AREA; --min-grain-area is a separate optional "
        "grain-size filter; --min-tj-filtered-gb-area is an optional extra. "
        "Velocity filters only the current frame and checks persistence in "
        "the raw next-frame gb_dict. Velocity normalization uses raw_gb_area/2. "
        "Changing the HDF5 TJ distance still requires regenerating Stage 1 GB "
        "properties, but not a different curvature algorithm."
    )
    ...
    if not args.skip_velocity:
        ...
        velocity_df = compute_gb_velocity_averaged(
            ...
            min_gb_area=args.min_gb_area,
            min_tj_filtered_gb_area=args.min_tj_filtered_gb_area,
            all_interval_results=flat_lists["all_interval_results"],
            log=log,
        )
        ...
        # ── 6.2/6.3 Path 2 00100 filtering and classification ────────────────
        normc_flat, antic_flat, classification_diagnostics = (
            build_path2_velocity_subsets(
                flat_lists,
                use_isolated_filter=args.isolated_antic_filter,
                log=log,
            )
        )
        flat_lists["classification_diagnostics"] = classification_diagnostics
